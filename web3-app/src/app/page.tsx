    ...
    try {
      ...
      if ((e as { code?: number }).code === 4902) {
        await (window as { ethereum?: { request: (args: { method: string; params?: unknown[] }) => Promise<void> } }).ethereum?.request({
          ...
          params: [{ chainId: '0x' + SUPPORTED_CHAIN_ID.toString(16), chainName: 'Polkadot Hub Testnet', nativeCurrency: { name: 'PAS', symbol: 'PAS', decimals: 18 }, rpcUrls: ['https://services.polkadothub-rpc.com/testnet'], blockExplorerUrls: ['https://blockscout-passet-hub.parity-testnet.parity.io'] }]
        });
      }
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8', color: '#111827', fontFamily: "'Sora', 'DM Sans', system-ui, sans-serif", overflowX: 'hidden' }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-link::after {
          ...
          height: 2px; background: var(--amber-light); border-radius: 2px;
        ...
        .card-amber {
          ...
          border-color: rgba(201,134,10,0.2);
        }
        ...
        .btn-amber {
          ...
          box-shadow: 0 4px 14px rgba(201,134,10,0.3);
          white-space: nowrap;
        }
        ...
        .price-chip {
          ...
          box-shadow: var(--shadow-xs); transition: border-color 0.2s;
        }
        ...
        .grid-bg {
          ...
          background-image: linear-gradient(rgba(201,134,10,0.04) 1px, transparent 1px),
        ...
        @media (max-width: 1100px) {
          ...
          .hero-right-col { width: 100% !important; max-width: 520px; margin: 0 auto; }
        ...
        @media (max-width: 768px) {
          ...
          .hero-flex { gap: 36px !important; margin-bottom: 48px !important; }
          .card { border-radius: 18px !important; }
          .step-card { padding: 16px !important; }
          .feat-card { padding: 20px !important; }
          .stat-badge { padding: 16px 12px !important; }
          .btn-primary, .btn-amber, .btn-ghost { font-size: 14px !important; padding: 12px 20px !important; }
          .price-chip { padding: 6px 10px !important; }
        }
        @media (max-width: 400px) {
          .hero-h1 { font-size: 1.9rem !important; }
          .stats-row { grid-template-columns: repeat(2,1fr) !important; gap: 10px !important; }
          main { padding: 24px 14px 56px !important; }
        }
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          <button
            onClick={claimFaucet}
            disabled={!isReady || faucetState !== 'idle'}
            className="desktop-only btn-amber"
            style={{ padding: '9px 18px', fontSize: 13, borderRadius: 10, boxShadow: '0 2px 10px rgba(201,134,10,0.2)' }}
          >
            {faucetState === 'pending' || faucetState === 'confirming'
              ? <><div className="spinner" />&nbsp;Minting…</>
              : faucetState === 'done'
              ? '✓ USDC Received'
              : !isReady
              ? '🪙 USDC Faucet'
          ...
          <a
            ...
            className="desktop-only btn-ghost"
            style={{ padding: '9px 16px', fontSize: 13, borderRadius: 10, gap: 6, textDecoration: 'none' }}
          >
            🔴 PAS Faucet ↗
          </a>
      ...
      <div className={`mobile-menu-overlay ${isMobileMenuOpen ? 'open' : ''}`}>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 12, paddingTop: 32 }}>
          <button onClick={claimFaucet} disabled={!isReady || faucetState !== 'idle'} className="btn-amber" style={{ width: '100%', padding: '14px' }}>
            {faucetLabel}
          </button>
            ...
            tickerItems.map((item, i) => {
              const inner = (
                <div key={`${gi}-${i}`} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 28px', borderRight: '1px solid rgba(255,255,255,0.07)', height: '100%', flexShrink: 0 }}>
                  ...
                  {item.href && <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: '#FCD34D', fontWeight: 600, letterSpacing: '0.04em' }}>↗</span>}
                </div>
              );
              return item.href
                ? <a key={`${gi}-${i}`} href={item.href} target="_blank" rel="noopener noreferrer" className="ticker-link">{inner}</a>
                : inner;
            })
          )}
        </div>
      ...
      <main style={{ position: 'relative', zIndex: 10, maxWidth: 1240, margin: '0 auto', padding: 'clamp(56px,7vw,96px) 5% 96px' }}>
        ...
        <div className="hero-flex fade-up d1" style={{ display: 'flex', gap: 64, alignItems: 'center', justifyContent: 'space-between', marginBottom: 96, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 300 }}>
            ...
            <h1 className="hero-h1" style={{ fontSize: 'clamp(3rem, 5.5vw, 5rem)', fontWeight: 800, lineHeight: 1.06, letterSpacing: '-0.03em', marginBottom: 22, color: 'var(--ink)' }}>
              Trade Perps.<br />
              <span style={{ background: 'linear-gradient(135deg, #C9860A 0%, #F59E0B 60%, #C9860A 100%)', backgroundSize: '200% auto', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', animation: 'shimmer 4s linear infinite' }}>
                Own Your Edge.
              </span>
            </h1>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 36 }}>
              {[
                ...
                <div key={c.sym} className="price-chip">
                  ...
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 700, color: c.up ? 'var(--green)' : 'var(--red)', background: c.up ? 'var(--green-pale)' : 'var(--red-pale)', padding: '2px 7px', borderRadius: 5 }}>
                    {c.up ? '▲ ' : '▼ '}{c.ch}
                  </span>
                </div>
              ))}
            ...
            <div className="card" style={{ padding: 28, borderRadius: 'var(--radius-xl)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div className={`step-card ${isConnected && !isWrong ? 'done' : !isConnected ? 'active' : ''}`}>
                  <div className="step-num" style={{ background: isConnected && !isWrong ? 'var(--green)' : 'var(--ink)' }}>
                    {isConnected && !isWrong ? '✓' : '1'}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)', marginBottom: 6 }}>
                      {isConnected && !isWrong ? 'Wallet Connected' : 'Connect Wallet'}
                    </div>
                    {isWrong
                ...
                <div className={`step-card ${faucetState === 'done' ? 'done' : isReady ? 'active' : ''}`} style={{ opacity: isReady ? 1 : 0.5 }}>
                  <div className="step-num" style={{ background: faucetState === 'done' ? 'var(--green)' : 'var(--amber)' }}>
                    {faucetState === 'done' ? '✓' : '2'}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)', marginBottom: 2 }}>
                      {faucetState === 'done' ? '10,000 USDC Received!' : 'Get Testnet USDC'}
                    </div>
                    ...
                    >
                      {faucetState === 'pending' || faucetState === 'confirming'
                        ? <><div className="spinner" />&nbsp;{faucetState === 'pending' ? 'Confirm in wallet…' : 'Minting…'}</>
                        : faucetState === 'done' ? '✓ USDC Ready!'
                        : '🪙 Claim Free USDC'}
                    </button>
                    {/* Polkadot token faucet - visible button */}
                    <a
                      href="https://faucet.polkadot.io/"
                      target="_blank"
                      rel="noopener noreferrer"
        ...
        <div className="fade-up d3">
          <div style={{ textAlign: 'center', marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.8rem, 3.5vw, 2.6rem)', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.025em', lineHeight: 1.15, marginBottom: 14 }}>
              Built for Polkadot Hub
            </h2>
            <p style={{ fontSize: 16, color: 'var(--ink-3)', maxWidth: 520, margin: '0 auto', lineHeight: 1.6 }}>
              Secure, transparent, and completely non-custodial infrastructure for perpetual trading.
            </p>
          </div>
        ...
        <div className="fade-up d4" style={{ marginTop: 80 }}>
          <div className="card card-amber" style={{ padding: 'clamp(36px,5vw,56px)', textAlign: 'center', borderRadius: 'var(--radius-xl)' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.6rem, 3vw, 2.4rem)', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.025em', marginBottom: 14 }}>
              Start trading in under 60 seconds
            </h2>
            <p style={{ fontSize: 16, color: 'var(--ink-3)', maxWidth: 480, margin: '0 auto 32px', lineHeight: 1.6 }}>
              Connect your wallet, claim free USDC from the faucet, and open your first leveraged position on Polkadot Hub.
            </p>
